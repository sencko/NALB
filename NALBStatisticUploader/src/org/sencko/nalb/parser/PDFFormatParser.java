...
public class PDFFormatParser {
  ...
  public static void main(String[] args) throws Exception {// PDFText2HTML
    ...
    for (File file : files) {
      try {
        ...
        Formatter formatter = new Formatter("results/" + game.homeTeam.getTeam().getAlias() + "-" + game.awayTeam.getTeam().getAlias() + game.gameNo
            + ".html", "UTF-8");
    ...
    try {
      ...
      allTeam.toHtml(formatter, false);
      formatter.close();
    } catch (Exception ex) {
      ex.printStackTrace();
    }

    generateTotals(allTeam);

    generateAllStars();
  }

  private static void generateAllStars() {
    Team allTeam = new Team("ALL-STARS", "Звезди");
    Comparator<Player> playerComparator = new TotalCoefficientComparator();
    for (Team team : Team.teamSet) {

      ArrayList<Player> arrayList = new ArrayList<Player>();
      arrayList.addAll(team.playerSet);
      Collections.sort(arrayList, playerComparator);
      allTeam.playerSet.add(arrayList.get(0));
      allTeam.playerSet.add(arrayList.get(1));
    }

    for (Player player : allTeam.playerSet) {
      player.realName = player.realName + "(" + player.team.getName() + ")";
    }

    try {
      Formatter formatter = new Formatter("results/" + allTeam.getAlias() + ".html", "UTF-8");
      allTeam.toHtml(formatter, true);
      formatter.close();
    } catch (Exception ex) {
      ex.printStackTrace();
    }
  ...
  private static void generateTotals(Team allTeam) {
    for (Team team : Team.teamSet) {
      try {
        Formatter formatter = new Formatter("results/" + team.getAlias() + "_totals.html", "UTF-8");
        team.toHtml(formatter, true);
        formatter.close();
      } catch (Exception ex) {
    ...
    try {
      Formatter formatter = new Formatter("results/" + allTeam.getAlias() + "_totals.html", "UTF-8");
      allTeam.toHtml(formatter, true);
      formatter.close();
    } catch (Exception ex) {
      ex.printStackTrace();
