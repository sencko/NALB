...
public class CMAPEncodingEntry {
  ...
  protected void processSubtype12(TrueTypeFont ttf, TTFDataStream data, int numGlyphs) throws IOException {
    ...
    for (long i = 0; i <= nbGroups; ++i) {
      ...
      if ((endCode > 0) && ((endCode < firstCode) || (endCode > 0x0010FFFF) || ((endCode >= 0x0000D800) && (endCode <= 0x0000DFFF)))) {
        throw new IOException("Invalid Characters codes");
      }
  ...
  protected void processSubtype4(TrueTypeFont ttf, TTFDataStream data, int numGlyphs) throws IOException {
      ...
      if ((start != 65535) && (end != 65535)) {
        for (int j = start; j <= end; j++) {
          if (rangeOffset == 0) {
            ...
            long glyphOffset = currentPosition + (((rangeOffset / 2) + // idRangeOffset[i]/2
    ...
    /*
     * this is the final result key=glyphId, value is character codes Create an array that contains MAX(GlyphIds) element and fill this array with the
     * .notdef character
     */
