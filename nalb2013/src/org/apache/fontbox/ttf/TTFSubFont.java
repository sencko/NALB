...
/**
 * A font, which is comprised of a subset of characters of a TrueType font. Based on code developed by Wolfgang Glas
...
public class TTFSubFont {
  ...
  private static boolean replicateNameRecord(NameRecord nr) {
    return (nr.getPlatformId() == NameRecord.PLATFORM_WINDOWS) && (nr.getPlatformEncodingId() == NameRecord.PLATFORM_ENCODING_WINDOWS_UNICODE)
  ...
  private byte[] buildOS2Table() throws IOException {
     ...
     * 
     * SHORT sTypoAscender SHORT sTypoDescender SHORT sTypoLineGap USHORT usWinAscent USHORT usWinDescent
     */
    ...
    try {
      ...
      for (Integer glyphId : this.glyphIds) {
        ...
        if ((buf.length >= 2) && (buf[0] == -1) && (buf[1] == -1)) {
           ...
           * 
           * int16 numberOfContours If the number of contours is positive or zero, it is a single glyph; If the number of contours is -1, the glyph is
           * compound FWord xMin Minimum x for coordinate data FWord yMin Minimum y for coordinate data FWord xMax Maximum x for coordinate data FWord
           * yMax Maximum y for coordinate data (here follow the data for the simple or compound glyph)
           * 
           * Table 15: Simple glyph definition Type Name Description uint16 endPtsOfContours[n] Array of last points of each contour; n is the number
           * of contours; array entries are point indices uint16 instructionLength Total number of bytes needed for instructions uint8
           * instructions[instructionLength] Array of instructions for this glyph uint8 flags[variable] Array of flags uint8 or int16 xCoordinates[]
           * Array of x-coordinates; the first is relative to (0,0), others are relative to previous point uint8 or int16 yCoordinates[] Array of
           * y-coordinates; the first is relative to (0,0), others are relative to previous point
           */
  ...
  private byte[] buildCmapTable() throws IOException {
    ...
    writeUint16(dos, 1);
    /*
     * UInt16 platformID Platform identifier UInt16 platformSpecificID Platform-specific encoding identifier UInt32 offset Offset of the mapping table
     */
    ...
    /*
     * UInt16 format Format number is set to 4 UInt16 length Length of subtable in bytes UInt16 language Language code for this encoding subtable, or
     * zero if language-independent UInt16 segCountX2 2 * segCount UInt16 searchRange 2 * (2**FLOOR(log2(segCount))) UInt16 entrySelector
     * log2(searchRange/2) UInt16 rangeShift (2 * segCount) - searchRange UInt16 endCode[segCount] Ending character code for each segment, last =
     * 0xFFFF. UInt16 reservedPad This value should be zero UInt16 startCode[segCount] Starting character code for each segment UInt16
     * idDelta[segCount] Delta for all character codes in segment UInt16 idRangeOffset[segCount] Offset in bytes to glyph indexArray, or 0 UInt16
     * glyphIndexArray[variable] Glyph index array
     */
  ...
  private byte[] buildPostTable() throws IOException {
    ...
    /*
     * Fixed format Format of this table Fixed italicAngle Italic angle in degrees FWord underlinePosition Underline position FWord underlineThickness
     * Underline thickness uint32 isFixedPitch Font is monospaced; set to 1 if the font is monospaced and 0 otherwise (N.B., to maintain compatibility
     * with older versions of the TrueType spec, accept any non-zero value as meaning that the font is monospaced) uint32 minMemType42 Minimum memory
     * usage when a TrueType font is downloaded as a Type 42 font uint32 maxMemType42 Maximum memory usage when a TrueType font is downloaded as a
     * Type 42 font uint32 minMemType1 Minimum memory usage when a TrueType font is downloaded as a Type 1 font uint32 maxMemType1 Maximum memory
     * usage when a TrueType font is downloaded as a Type 1 font uint16 numberOfGlyphs number of glyphs uint16 glyphNameIndex[numberOfGlyphs] Ordinal
     * number of this glyph in 'post' string tables. This is not an offset. Pascal string names[numberNewGlyphs] glyph names with length bytes
     * [variable] (a Pascal string)
     */
    writeFixed(dos, 2.0);
  ...
  public void writeToStream(OutputStream os) throws IOException {
    ...
    try {
      /*
       * 'cmap' character to glyph mapping 'glyf' glyph data 'head' font header 'hhea' horizontal header 'OS/2' OS/2 compatibility table. 'hmtx'
       * horizontal metrics 'loca' index to location 'maxp' maximum profile 'name' naming 'post' PostScript
       */
