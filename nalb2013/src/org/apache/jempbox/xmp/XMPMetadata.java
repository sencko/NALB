...
public class XMPMetadata {
  ...
  public XMPMetadata() throws IOException {
    ...
    ProcessingInstruction beginXPacket = xmpDocument.createProcessingInstruction("xpacket", "begin=\"\uFEFF\" id=\"W5M0MpCehiHzreSzNTczkc9d\"");
  ...
  private void init() {
    ...
    nsMappings.put(XMPSchemaBasicJobTicket.NAMESPACE, XMPSchemaBasicJobTicket.class);
    nsMappings.put(XMPSchemaDynamicMedia.NAMESPACE, XMPSchemaDynamicMedia.class);
    nsMappings.put(XMPSchemaPagedText.NAMESPACE, XMPSchemaPagedText.class);
    nsMappings.put(XMPSchemaIptc4xmpCore.NAMESPACE, XMPSchemaIptc4xmpCore.class);
   ...
   * 
   * @param namespace The namespace URI of the schmema for instance http://purl.org/dc/elements/1.1/.
   * @param xmpSchema The schema to associated this identifier with.
   */
   ...
   * 
   * @param outStream The stream to save the XMP document to.
   * 
  ...
  /**
   * Create and add a new PDF Schema to this metadata. Typically a XMP document will only have one PDF schema (but multiple are supported) so it is
   * recommended that you first check the existence of a PDF scheme by using getPDFSchema()
   * 
  ...
  /**
   * Create and add a new Dublin Core Schema to this metadata. Typically a XMP document will only have one schema for each type (but multiple are
   * supported) so it is recommended that you first check the existence of a this scheme by using getDublinCoreSchema()
   * 
   * @return A new blank PDF schema that is now part of the metadata.
   */
  ...
  /**
   * Create and add a new Basic Schema to this metadata. Typically a XMP document will only have one schema for each type (but multiple are supported)
   * so it is recommended that you first check the existence of a this scheme by using getDublinCoreSchema()
   * 
   * @return A new blank PDF schema that is now part of the metadata.
   */
   ...
   * 
   * @param is The input source to load the schema from.
   * 
    ...
    for (int i = 0; i < schemaList.getLength(); i++) {
      ...
      for (int j = 0; j < attributes.getLength(); j++) {
        ...
        if (name.startsWith("xmlns:") && nsMappings.containsKey(value)) {
          ...
          try {
            Constructor<?> ctor = schemaClass.getConstructor(new Class[] { Element.class, String.class });
            retval.add((XMPSchema) ctor.newInstance(new Object[] { schema, name.substring(6) }));
            found = true;
          } catch (NoSuchMethodException e) {
            throw new IOException("Error: Class " + schemaClass.getName() + " must have a constructor with the signature of " + schemaClass.getName()
  ...
  /**
   * Will return all schemas that fit the given namespaceURI. Which is only done by using the namespace mapping (nsMapping) and not by actually
   * checking the xmlns property.
   * 
   * @param namespaceURI The namespaceURI to filter for.
   * @return A list containing the found schemas or an empty list if non are found or the namespaceURI could not be found in the namespace mapping.
  ...
  public boolean hasUnknownSchema() throws IOException {
    ...
    for (int i = 0; i < schemaList.getLength(); i++) {
      ...
      for (int j = 0; j < attributes.getLength(); j++) {
        ...
        String name = attribute.getNodeName();
        String value = attribute.getNodeValue();
        if (name.startsWith("xmlns:") && !nsMappings.containsKey(value) && !value.equals(ResourceEvent.NAMESPACE)) {
          return true;
        }
      }
    }
   ...
   * 
   * @param targetSchema Class for targetSchema.
   * 
  ...
  public void merge(XMPMetadata metadata) throws IOException {
    ...
    for (Iterator<XMPSchema> iterator = schemas2.iterator(); iterator.hasNext();) {
      ...
      if (schema1 == null) {
        Element rdf = getRDFElement();
        rdf.appendChild(xmpDocument.importNode(schema2.getElement(), true));
      } else {
