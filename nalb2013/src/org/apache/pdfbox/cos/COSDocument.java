...
public class COSDocument extends COSBase {
  ...
  /**
   * Maps ObjectKeys to a COSObject. Note that references to these objects are also stored in COSDictionary objects that map a name to a specific
   * object.
   */
  ...
  /**
   * Constructor that will use the given random access file for storage of the PDF streams. The client of this method is responsible for deleting the
   * storage if necessary that this file will write to. The close method will close the file though.
   * 
   * @param scratchFileValue the random access file to use for storage
   * @param forceParsingValue flag to skip malformed or otherwise unparseable document content where possible
   */
  ...
  /**
   * Constructor that will use a temporary file in the given directory for storage of the PDF streams. The temporary file is automatically removed
   * when this document gets closed.
   * 
  ...
  /**
   * Constructor that will use the following random access file for storage of the PDF streams. The client of this method is responsible for deleting
   * the storage if necessary that this file will write to. The close method will close the file though.
   * 
  ...
  /**
   * Signals that the document is decrypted completely. Needed e.g. by {@link NonSequentialPDFParser} to circumvent additional decryption later on.
   */
  ...
  /**
   * Warn the user in the finalizer if he didn't close the PDF document. The method also closes the document just in case, to avoid abandoned
   * temporary files. It's still a good idea for the user to close the PDF document at the earliest possible to conserve resources.
   * 
  ...
  /**
   * Controls whether this instance shall issue a warning if the PDF document wasn't closed properly through a call to the {@link #close()} method. If
   * the PDF document is held in a cache governed by soft references it is impossible to reliably close the document before the warning is raised. By
   * default, the warning is enabled.
   * 
  ...
  /**
   * This method will search the list of objects for types of ObjStm. If it finds them then it will parse out all of the objects from the stream that
   * is contains.
   * 
  ...
  public void dereferenceObjectStreams() throws IOException {
    for (COSObject objStream : getObjectsByType(COSName.OBJ_STM)) {
      ...
      PDFObjectStreamParser parser = new PDFObjectStreamParser(stream, this, forceParsing);
      parser.parse();
