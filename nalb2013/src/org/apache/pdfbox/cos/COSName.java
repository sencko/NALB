...
public final class COSName extends COSBase implements Comparable<COSName> {
  ...
  /**
   * All common COSName values are stored in a simple HashMap. They are already defined as static constants and don't need to be synchronized for
  ...
  public void writePDF(OutputStream output) throws IOException {
    ...
    for (int i = 0; i < bytes.length; i++) {
      ...
      if ((current <= 32) || (current >= 127) || (current == '(') || (current == ')') || (current == '[') || (current == ']') || (current == '/')
          || (current == '%') || (current == '<') || (current == '>') || (current == NAME_ESCAPE[0])) {
  ...
  /**
   * Not usually needed except if resources need to be reclaimed in a ong running process. Patch provided by flester@GMail.com incorporated 5/23/08,
   * Danielwilson@users.SourceForge.net
   */
