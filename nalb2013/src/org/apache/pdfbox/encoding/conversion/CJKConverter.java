...
public class CJKConverter implements EncodingConverter {
  ...
  /**
   * Convert a string. It occurs when a cmap lookup returned converted bytes successfully, but we still need to convert its encoding. The parameter s
   * is constructs as one byte or a UTF-16BE encoded string.
   * 
   * Note: pdfbox set string to UTF-16BE charset before calling into this.
   * 
   * {@inheritDoc}
   */
  @Override
  ...
  /**
   * Convert bytes to a string. We just convert bytes within coderange defined in CMap.
   * 
   * {@inheritDoc}
   */
  @Override
