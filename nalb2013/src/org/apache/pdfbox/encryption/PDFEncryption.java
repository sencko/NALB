...
public final class PDFEncryption {
   ...
   */
  public final void encryptData(long objectNumber, long genNumber, byte[] key, InputStream data, OutputStream output) throws CryptographyException,
      IOException {
  ...
  public final byte[] getUserPassword(byte[] ownerPassword, byte[] o, int revision, long length) throws CryptographyException, IOException {
    try {
      ...
      if (revision == 2) {
        ...
        /**
         * byte[] iterationKey = new byte[ rc4Key.length ]; byte[] dataToEncrypt = o; for( int i=19; i>=0; i-- ) { System.arraycopy( rc4Key, 0,
         * iterationKey, 0, rc4Key.length ); for( int j=0; j< iterationKey.length; j++ ) { iterationKey[j] = (byte)(iterationKey[j] ^ (byte)i); }
         * rc4.setKey( iterationKey ); rc4.write( dataToEncrypt, result ); dataToEncrypt = result.toByteArray(); result.reset(); } result.write(
   ...
   */
  public final byte[] computeOwnerPassword(byte[] ownerPassword, byte[] userPassword, int revision, int length) throws CryptographyException,
      IOException {
    try {
      // STEP 1
      byte[] ownerPadded = truncateOrPad(ownerPassword);
