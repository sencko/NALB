...
/**
 * This is the used for the LZWDecode filter. This represents the dictionary mappings between codes and their values.
 * 
...
final class LZWDictionary {
  ...
  public void visit(byte data) throws IOException {
    ...
    if (current == null) {
      ...
      /**
       * System.out.print( "Adding " + code + "='" ); for( int i=0; i<bufferNextWrite; i++ ) { String hex = Integer.toHexString( ((buffer[i]&0xFF );
       * if( hex.length() <=1 ) { hex = "0" + hex; } if( i != bufferNextWrite -1 ) { hex += " "; } System.out.print( hex.toUpperCase() ); }
       * System.out.println( "'" );
       **/
      bufferNextWrite = 0;
      current = root;
