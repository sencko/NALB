 ...
 * 
 * The inheritance from PushBackInputStream is only to avoid the introduction of an interface with all PushBackInputStream methods. The parent
 * PushBackInputStream is not used in any way and all methods are overridden. (Thus when adding new methods to PushBackInputStream override them in
 * this class as well!) unread() is limited to the number of bytes already read from this stream (i.e. the current position in the array). This
 * limitation usually poses no problem to a parser, but allows for some optimization since only one array has to be dealt with.
 * 
 * Note: This class is not thread safe. Clients must provide synchronization if needed.
 * 
 * Note: Calling unread() after mark() will cause (part of) the unread data to be read again after reset(). Thus do not call unread() between mark()
 * and reset().
 * 
...
public class ByteArrayPushBackInputStream extends PushBackInputStream {
   ...
   * Constructor.
   * 
   * @param input Data to read from. Note that calls to unread() will modify this array! If this is not desired, pass a copy.
   * 
  ...
  /**
   * Pushes back a portion of an array of bytes by copying it to the front of the pushback buffer. After this method returns, the next byte to be read
   * will have the value b[off], the byte after that will have the value b[off+1], and so forth.
   * 
   * @param buffer the byte array to push back.
   * @param off the start offset of the data.
   * @param len the number of bytes to push back.
   * @throws IOException If there is not enough room in the pushback buffer for the specified number of bytes.
  ...
  /**
   * Pushes back a portion of an array of bytes by copying it to the front of the pushback buffer. After this method returns, the next byte to be read
   * will have the value buffer[0], the byte after that will have the value buffer[1], and so forth.
   * 
   * @param buffer the byte array to push back.
   * @throws IOException If there is not enough room in the pushback buffer for the specified number of bytes.
  ...
  /**
   * Pushes back a portion of an array of bytes by copying it to the front of the pushback buffer. After this method returns, the next byte to be read
   * will have the value buffer[off], the byte after that will have the value buffer[off+1], and so forth. Internal method that assumes off and len to
   * be valid.
   * 
   * @param buffer the byte array to push back.
   * @param off the start offset of the data.
  ...
  /**
   * Skips over and discards n bytes of data from this input stream. The skip method may, for a variety of reasons, end up skipping over some smaller
   * number of bytes, possibly 0. This may result from any of a number of conditions; reaching end of file before n bytes have been skipped is only
   * one possibility. The actual number of bytes skipped is returned. If n is negative, no bytes are skipped.
   * 
   ...
   * 
   * @param newpos Position in the underlying array. A negative value will be interpreted as 0, a value greater than size() as size().
   * @return old position.
   */
