...
public class PushBackInputStream extends java.io.PushbackInputStream {
  ...
  /**
   * In case provided input stream implements {@link RandomAccessRead} we hold a typed reference to it in order to support seek operations.
   */
  ...
  /**
   * This is a method used to fix PDFBox issue 974661, the PDF parsing code needs to know if there is at least x amount of data left in the stream,
   * but the available() method returns how much data will be available without blocking. PDFBox is willing to block to read the data, so we will
   * first fill the internal buffer.
   * 
