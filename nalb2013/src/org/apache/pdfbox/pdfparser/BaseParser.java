...
public abstract class BaseParser {
   ...
   * @param forceParsingValue flag to skip malformed or otherwise unparseable input where possible
   * @throws IOException If there is an error reading the input stream.
   */
  public BaseParser( InputStream input, boolean forceParsingValue) throws IOException {
    this.pdfSource = new PushBackInputStream(new BufferedInputStream(input, 16384), Integer.getInteger(PROP_PUSHBACK_SIZE, 65536));
  ...
  protected COSStream parseCOSStream(COSDictionary dic, RandomAccess file) throws IOException {
    ...
    try {
        ...
        if (readCount > 0) {
          ...
          if (!foundEndstream) {
            ...
            try {
              ...
              throw new WrappedIOException("Could not push back " + bout.size() + " bytes in order to reparse stream. "
                  + "Try increasing push back buffer using system property " + PROP_PUSHBACK_SIZE, ioe);
            }
      ...
      if (!endStream.equals(ENDSTREAM_STRING)) {
        /*
         * Sometimes stream objects don't have an endstream tag so readUntilEndStream(out) also can stop on endobj tags. If that's the case we need to
         * make sure to unread the endobj so parseObject() can handle that case normally.
         */
        ...
        /*
         * Some PDF files don't contain a new line after endstream so we need to make sure that the next object number is getting read separately and
         * not part of the endstream keyword. Ex. Some files would have "endstream8" instead of "endstream"
         */
          ...
          /*
           * If for some reason we get something else here, Read until we find the next "endstream"
           */
          readUntilEndStream(out);
  ...
  /**
   * This method will read through the current stream object until we find the keyword "endstream" meaning we're at the end of this object. Some pdf
   * files, however, forget to write some endstream tags and just close off objects with an "endobj" tag so we have to handle this case as well.
   * 
  ...
  /**
   * This is really a bug in the Document creators code, but it caused a crash in PDFBox, the first bug was in this format: /Title ( (5) /Creator
   * which was patched in 1 place. However it missed the case where the Close Paren was escaped
   * 
...
/**
  ...
  protected boolean isEndOfName(char ch) {
    return ((ch == ' ') || (ch == 13) || (ch == 10) || (ch == 9) || (ch == '>') || (ch == '<') || (ch == '[') || (ch == '/') || (ch == ']')
        || (ch == ')') || (ch == '(') || (ch == -1 // EOF
    ));
  }
      ...
      default: {
        if (Character.isDigit(c) || (c == '-') || (c == '+') || (c == '.')) {
          ...
          if ((badString == null) || (badString.length() == 0)) {
            ...
            throw new IOException("Unknown dir object c='" + c + "' cInt=" + (int) c + " peek='" + (char) peek + "' peekInt=" + peek + " "
                + pdfSource.getOffset());
          }
  ...
  protected String readString(int length) throws IOException {
    ...
    while (!isWhitespace(c) && !isClosing(c) && (c != -1) && (buffer.length() < length) && (c != '[') && (c != '<') && (c != '(') && (c != '/')) {
      buffer.append((char) c);
      c = pdfSource.read();
    }
    if (c != -1) {
  ...
  /**
   * This will read a long from the Stream and throw an {@link IllegalArgumentException} if the long value has more than 10 digits (i.e. : bigger than
  ...
  /**
   * This will read a integer from the Stream and throw an {@link IllegalArgumentException} if the integer value has more than the maximum object
   * revision (i.e. : bigger than {@link #GENERATION_NUMBER_THRESHOLD})
   * 
   * @return
   * @throws IOException
   */
