...
public class ConformingPDFParser extends BaseParser {
  ...
  /**
   * This will read all bytes (backwards) until a non-whitespace character is found. To save you an extra read, the non-whitespace character is
   * returned. If the current character is not whitespace, this method will just return the current char.
   * 
  ...
  /**
   * This will read all bytes until a non-whitespace character is found. To save you an extra read, the non-whitespace character is returned. If the
   * current character is not whitespace, this method will just return the current char.
   * 
   * @return the first non-whitespace character found
   * @throws IOException if there is an error reading from the file
   */
  ...
  /**
   * This will consume any whitespace, read in bytes until whitespace is found again and then parse the characters which have been read as a long. The
   * current offset will then point at the first whitespace character which preceeds the number.
   * 
  ...
  /**
   * This will read an object from the inputFile at whatever our currentOffset is. If the object and generation are not the expected values and this
   * object is set to throw an exception for non-conforming documents, then an exception will be thrown.
   * 
  ...
  public COSBase readObject(long objectNumber, long generation) throws IOException {
    ...
    if (objectNumber != actualObjectNumber) {
      if (throwNonConformingException) {
        throw new AssertionError("Object numer expected was " + objectNumber + " but actual was " + actualObjectNumber);
      }
    }
    consumeWhitespace();
    ...
    if (generation != actualGeneration) {
      if (throwNonConformingException) {
        throw new AssertionError("Generation expected was " + generation + " but actual was " + actualGeneration);
      }
    }
    consumeWhitespace();
  ...
  /**
   * This will read a line starting with the byte at offset and going backwards until it finds a newline. This should only be used if we are certain
   * that the data will only be text, and not binary data.
   * 
  ...
  /**
   * This will read a line starting with the byte at offset and going forward until it finds a newline. This should only be used if we are certain
   * that the data will only be text, and not binary data.
   * 
   * @param offset the location of the file where we should start reading
   * @return the string which was read
  ...
  protected String readWord() throws IOException {
    ...
    do {
      ...
      if (!stop && (sb.length() > 0)) {
        stop = (singleByte == '/') || (singleByte == '[') || (singleByte == ']') || ((singleByte == '>') && !">".equals(sb.toString()));
        if (stop) {
