...
public class PDFParser extends BaseParser {
  ...
  /**
   * A list of duplicate objects found when Parsing the PDF File.
   */
  ...
  /**
   * This is the directory where pdfbox will create a temporary file for storing pdf document stream in. By default this directory will be the value
   * of the system property java.io.tmpdir.
   * 
   * @param tmpDir The directory to create scratch files needed to store pdf document streams.
   */
  ...
  /**
   * Returns true if parsing should be continued. By default, forceParsing is returned. This can be overridden to add application specific handling
   * (for example to stop parsing when the number of exceptions thrown exceed a certain number).
   * 
    ...
    try {
      ...
      while (true) {
        ...
        try {
          ...
          /*
           * PDF files may have random data after the EOF marker. Ignore errors if last object processed is EOF.
           */
          ...
          if (isContinueOnError(e)) {
            /*
             * Warning is sent to the PDFBox.log and to the Console that we skipped over an object
             */
            ...
            /*
             * the nextObject is the one we want to skip so read the 'Object Number' without interpret it in order to force the skipObject
             */
  ...
  /**
   * Skip to the start of the next object. This is used to recover from a corrupt object. This should handle all cases that parseObject supports. This
   * assumes that the next object will start on its own line.
   * 
   * @throws IOException
   */
  private void skipToNextObj() throws IOException {
    ...
    /*
     * Read a buffer of data each time to see if it starts with a known keyword. This is not the most efficient design, but we should rarely be
     * needing this function. We could update this to use the circular buffer, like in readUntilEndStream().
     */
  ...
  /**
   * This will get the document that was parsed. parse() must be called before this is called. When you are done with this document you must call
   * close() on it to release resources.
   * 
  ...
  private boolean parseObject() throws IOException {
    ...
    // we are going to parse an normal object
      ...
      /*
       * If the object we returned already has a baseobject, then we have a conflict which we will resolve using information after we parse the xref
       * table.
       */
      else {
      ...
      if (!endObjectKey.equals("endobj")) {
        if (endObjectKey.startsWith("endobj")) {
          /*
           * Some PDF files don't contain a new line after endobj so we need to make sure that the next object number is getting read separately and
           * not part of the endobj keyword. Ex. Some files would have "endobj28" instead of "endobj"
           */
          ...
          /*
           * Some PDF files contain junk (like ">> ", in the case of a PDF I found which was created by Exstream Dialogue Version 5.0.039) in which
           * case we ignore the data before endobj and just move on
           */
  ...
  protected boolean parseStartXref() throws IOException {
    ...
    skipSpaces();
    /*
     * This integer is the byte offset of the first object referenced by the xref or xref stream Needed for the incremental update (PREV)
     */
  ...
  /**
   * Used to resolve conflicts when a PDF Document has multiple objects with the same id number. Ideally, we could use the Xref table when parsing the
   * document to be able to determine which of the objects with the same ID is correct, but we do not have access to the Xref Table during parsing.
   * Instead, we queue up the conflicts and resolve them after the Xref has been parsed. The Objects listed in the Xref Table are kept and the others
   * are ignored.
   */
  private static class ConflictObj {
    ...
    /**
     * Sometimes pdf files have objects with the same ID number yet are not referenced by the Xref table and therefore should be excluded. This method
     * goes through the conflicts list and replaces the object stored in the objects array with this one if it is referenced by the xref table.
     * 
     * @throws IOException
     */
          ...
          if (tolerantConflicResolver(values, offset, 4)) {
            ...
            if ((pdfObject.getObjectNumber() != null) && pdfObject.getObjectNumber().equals(o.object.getObjectNumber())) {
              ...
              LOG.debug("Conflict object [" + o.objectKey + "] at offset " + offset
                  + " found in the xref table, but the object numbers differ. Ignoring this object." + " The document is maybe malformed.");
            }
          }
  ...
  /**
   * Check if the given object offset can be find in the xref table. If not, we try to search the table again with the given tolerance and check the
   * given bytes before and after the xref table offset.
   * 
