...
/**
 * This class will collect all XRef/trailer objects and creates correct xref/trailer information after all objects are read using startxref and 'Prev'
 * information (unused XRef/trailer objects are discarded).
 * 
 * In case of missing startxref or wrong startxref pointer all XRef/trailer objects are used to create xref table / trailer dictionary in order they
 * occur.
 * 
 * For each new xref object/XRef stream method {@link #nextXrefObj(int)} must be called with start byte position. All following calls to
 * {@link #setXRef(COSObjectKey, int)} or {@link #setTrailer(COSDictionary)} will add the data for this byte position.
 * 
 * After all objects are parsed the startxref position must be provided using {@link #setStartxref(int)}. This is used to build the chain of active
 * xref/trailer objects used for creating document trailer and xref table.
 * 
...
public class XrefTrailerResolver {
  ...
  /**
   * Sets the byte position of the first XRef (has to be called after very last startxref was read). This is used to resolve chain of active
   * XRef/trailer.
   * 
   * In case startxref position is not found we output a warning and use all XRef/trailer objects combined in byte position order. Thus for incomplete
   * PDF documents with missing startxref one could call this method with parameter value -1.
   * 
  ...
  /**
   * Gets the resolved trailer. Might return <code>null</code> in case {@link #setStartxref(int)} was not called before.
   * 
  ...
  /**
   * Gets the resolved xref table. Might return <code>null</code> in case {@link #setStartxref(int)} was not called before.
   * 
   ...
   * 
   * This will scan resolved xref table for all entries having negated stream object number as value.
   * 
   * @param objstmObjNr object number of object stream for which contained object numbers should be returned
   * 
   * @return set of object numbers referenced for given object stream or <code>null</code> if {@link #setStartxref(long)} was not called before so
   *         that no resolved xref table exists
   */
