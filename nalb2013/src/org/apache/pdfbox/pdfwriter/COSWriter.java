 ...
 * 
 * todo no support for incremental updates todo single xref section only todo no linearization
 * 
...
public class COSWriter implements ICOSVisitor {
  ...
  private void addObjectToWrite(COSBase object) {
    ...
    if (!writtenObjects.contains(object) && !objectsToWriteSet.contains(object) && !actualsAdded.contains(actual)) {
      ...
      if ((actual != null) && objectKeys.containsKey(actual) && !object.isNeedToBeUpdate() && ((cosBase != null) && !cosBase.isNeedToBeUpdate())) {
        return;
      }
  ...
  /**
   * check the xref entries and write out the ranges. The format of the returned array is exactly the same as the pdf specification. See section 7.5.4
   * of ISO32000-1:2008, example 1 (page 40) for reference.
   * <p>
   ...
   * 0 3 5 4 10 1
   * <p>
   * this mean that the element 0 is followed by two other related numbers that represent a cluster of the size 3. 5 is follow by three other related
   * numbers and create a cluster of size 4. etc.
   * 
