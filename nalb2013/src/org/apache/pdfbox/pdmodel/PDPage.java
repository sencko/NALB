 ...
 * <p>
 * This class implements the {@link Printable} interface, but since PDFBox version 1.3.0 you should be using the {@link PDPageable} adapter instead
...
public class PDPage implements COSObjectable {
  ...
  /**
   * This will get the resources at this page and not look up the hierarchy. This attribute is inheritable, and findResources() should probably used.
  ...
  /**
   * A rectangle, expressed in default user space units, defining the boundaries of the physical medium on which the page is intended to be displayed
   * or printed
   * 
   * This will get the MediaBox at this page and not look up the hierarchy. This attribute is inheritable, and findMediaBox() should probably used.
  ...
  /**
   * A rectangle, expressed in default user space units, defining the visible region of default user space. When the page is displayed or printed, its
   * contents are to be clipped (cropped) to this rectangle and then imposed on the output medium in some implementationdefined manner
   * 
   * This will get the CropBox at this page and not look up the hierarchy. This attribute is inheritable, and findCropBox() should probably used. This
   * will return null if no CropBox is available at this level.
   * 
  ...
  /**
   * This will search for a crop box in the parent and return null if it is not found. It will NOT default to the media box if it cannot be found.
   * 
   * @param node The node
   */
  ...
  /**
   * A rectangle, expressed in default user space units, defining the region to which the contents of the page should be clipped when output in a
   * production environment. The default is the CropBox.
   * 
  ...
  /**
   * A rectangle, expressed in default user space units, defining the intended dimensions of the finished page after trimming. The default is the
   * CropBox.
   * 
  ...
  /**
   * A rectangle, expressed in default user space units, defining the extent of the page's meaningful content (including potential white space) as
   * intended by the page's creator The default isthe CropBox.
   * 
  ...
  /**
   * A value representing the rotation. This will be null if not set at this level The number of degrees by which the page should be rotated clockwise
   * when displayed or printed. The value must be a multiple of 90.
   * 
   * This will get the rotation at this page and not look up the hierarchy. This attribute is inheritable, and findRotation() should probably used.
  ...
  /**
   * This will get the contents of the PDF Page, in the case that the contents of the page is an array then then the entire array of streams will be
  ...
  /**
   * This will get a list of PDThreadBead objects, which are article threads in the document. This will return an empty list of there are no thread
   * beads.
   * 
  ...
  /**
   * @deprecated Use the {@link PDPageable} adapter class {@inheritDoc}
   */
