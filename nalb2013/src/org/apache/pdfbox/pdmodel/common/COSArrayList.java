...
public class COSArrayList<E> implements List<E> {
  ...
  /**
   * This is a really special constructor. Sometimes the PDF spec says that a dictionary entry can either be a single item or an array of those items.
   * But in the PDModel interface we really just want to always return a java.util.List. In the case were we get the list and never modify it we don't
   * want to convert to COSArray and put one element, unless we append to the list. So here we are going to create this object with a single item
   * instead of a list, but allow more items to be added and then converted to an array.
   * 
