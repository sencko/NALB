 ...
 * 
 * @author <a href="mailto:igor.podolskiy@ievvwi.uni-stuttgart.de">Igor Podolskiy</a>
 * @version $Revision$
 */
public class PDPageLabels implements COSObjectable {
   ...
   * 
   * <p>
   * Note that the page label dictionary won't be automatically added to the document; you will still need to do it manually (see
   ...
   * 
   * <p>
   * This will be always &gt;= 1, as the required default entry for the page range starting at the first page is added automatically by this
  ...
  /**
   * Returns a mapping with computed page labels as keys and corresponding 0-based page indices as values. The returned map will contain at most as
   * much entries as the document has pages.
   * 
   * <p>
   * <strong>NOTE:</strong> If the document contains duplicate page labels, the returned map will contain <em>less</em> entries than the document has
   * pages. The page index returned in this case is the <em>highest</em> index among all pages sharing the same label.
   * </p>
   * 
  ...
  private void computeLabels(LabelHandler handler) {
    Iterator<Entry<Integer, PDPageLabelRange>> iterator = labels.entrySet().iterator();
    if (!iterator.hasNext()) {
      return;
    }
    int pageIndex = 0;
    ...
    LabelGenerator gen = new LabelGenerator(lastEntry.getValue(), doc.getNumberOfPages() - lastEntry.getKey());
    while (gen.hasNext()) {
      handler.newLabel(pageIndex, gen.next());
      pageIndex++;
    }
  ...
  private static class LabelGenerator implements Iterator<String> {
     ...
     */
    private static final String[][] ROMANS = new String[][] { { "", "i", "ii", "iii", "iv", "v", "vi", "vii", "viii", "ix" },
