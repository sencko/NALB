 ...
 * </ul>
 * 
 * This class can be used to protect a document by assigning access permissions to recipients. In this case, it must be used with a specific
 * ProtectionPolicy.
 * 
 * 
 * When a document is decrypted, it has a currentAccessPermission property which is the access permissions granted to the user who decrypted the
 * document.
 * 
...
public class AccessPermission {
  ...
  /**
   * This returns an integer representing the access permissions. This integer can be used for public key encryption. This format is not documented in
   * the PDF specifications but is necessary for compatibility with Adobe Acrobat and Adobe Reader.
   * 
  ...
  /**
   * The returns an integer representing the access permissions. This integer can be used for standard PDF encryption as specified in the PDF
   * specifications.
   * 
  ...
  /**
   * Set if the user can extract content from the document for accessibility purposes. This method will have no effect if the object is in read only
   * mode
   * 
   * @param allowExtraction A boolean determining if the user can extract content from the document.
   */
   ...
   * 
   * @param allowAssembly A boolean determining if the user can print the document in a degraded format.
   */
  ...
  /**
   * Locks the access permission read only (ie, the setters will have no effects). After that, the object cannot be unlocked. This method is used for
   * the currentAccessPermssion of a document to avoid users to change access permission.
   */
