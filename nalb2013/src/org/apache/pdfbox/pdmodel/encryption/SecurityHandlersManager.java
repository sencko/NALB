...
/**
 * This class manages security handlers for the application. It follows the singleton pattern. To be usable, security managers must be registered in
 * it. Security managers are retrieved by the application when necessary.
 * 
...
public class SecurityHandlersManager {
  ...
  /**
   * hashtable used to index handlers regarding their name. Basically this will be used when opening an encrypted document to find the appropriate
   * security handler to handle security features of the document.
   */
   ...
   * 
   * If the security handler was already registered an exception is thrown. If another handler was previously registered for the same filter name or
    ...
    if (SecurityHandler.class.isAssignableFrom(securityHandlerClass)) {
      try {
        ...
        if (handlerPolicyClasses.containsKey(protectionPolicyClass)) {
          throw new BadSecurityHandlerException("a security handler was already registered " + "for the policy class "
  ...
  public SecurityHandler getSecurityHandler(ProtectionPolicy policy) throws BadSecurityHandlerException {
    ...
    try {
      ...
      e.printStackTrace();
      throw new BadSecurityHandlerException("problem while trying to instanciate the security handler " + handlerclass.getName() + ": "
          + e.getMessage());
    }
  }

  /**
   * Retrieve the appropriate SecurityHandler for a the given filter name. The filter name is an entry of the encryption dictionary of an encrypted
   * document.
   * 
