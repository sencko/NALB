...
public class PDCIDFontType0Font extends PDCIDFont {
  ...
  /**
   * Returns the AWT font that corresponds with this CIDFontType0 font. By default we try to look up a system font with the same name. If that fails
   * and the font file is embedded in the PDF document, we try to generate the AWT font using the {@link PDType1CFont} class. Ideally the embedded
   * font would be used always if available, but since the code doesn't work correctly for all fonts yet we opt to use the system font by default.
   * 
