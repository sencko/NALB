...
public abstract class PDFont implements COSObjectable {
  ...
  /**
   * This will clear AFM resources that are stored statically. This is usually not a problem unless you want to reclaim resources for a long running
   * process.
   * 
   * SPECIAL NOTE: The font calculations are currently in COSObject, which is where they will reside until PDFont is mature enough to take them over.
   * PDFont is the appropriate place for them and not in COSObject but we need font calculations for text extraction. THIS METHOD WILL BE MOVED OR
   * REMOVED TO ANOTHER LOCATION IN A FUTURE VERSION OF PDFBOX.
   */
  ...
  /**
   * This will get the matrix that is used to transform glyph space to text space. By default there are 1000 glyph units to 1 text space unit, but
   * type3 fonts can use any value.
   * 
   * Note:If this is a type3 font then it can be modified via the PDType3Font.setFontMatrix, otherwise this is a read-only property.
   * 
