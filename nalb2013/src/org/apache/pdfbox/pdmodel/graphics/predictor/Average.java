...
public class Average extends PredictorAlgorithm {
  ...
  public void encodeLine(byte[] src, byte[] dest, int srcDy, int srcOffset, int destDy, int destOffset) {
    ...
    for (int x = 0; x < bpl; x++) {
      dest[x + destOffset] = (byte) (src[x + srcOffset] - ((leftPixel(src, srcOffset, srcDy, x) + abovePixel(src, srcOffset, srcDy, x)) >>> 2));
    }
  }
  ...
  public void decodeLine(byte[] src, byte[] dest, int srcDy, int srcOffset, int destDy, int destOffset) {
    int bpl = getWidth() * getBpp();
    for (int x = 0; x < bpl; x++) {
      dest[x + destOffset] = (byte) (src[x + srcOffset] + ((leftPixel(dest, destOffset, destDy, x) + abovePixel(dest, destOffset, destDy, x)) >>> 2));
    }
  }
}
