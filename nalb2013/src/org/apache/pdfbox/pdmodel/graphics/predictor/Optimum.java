...
public class Optimum extends PredictorAlgorithm {
  ...
  public void checkBufsiz(byte[] filtered, byte[] raw) {
    if (filtered.length != (((getWidth() * getBpp()) + 1) * getHeight())) {
      ...
      throw new IllegalArgumentException("filtered.length != (width*bpp + 1) * height, " + filtered.length + " "
          + (((getWidth() * getBpp()) + 1) * getHeight()) + "w,h,bpp=" + getWidth() + "," + getHeight() + "," + getBpp());
    }
    if (raw.length != (getWidth() * getHeight() * getBpp())) {
      throw new IllegalArgumentException("raw.length != width * height * bpp, raw.length=" + raw.length + " w,h,bpp=" + getWidth() + ","
          + getHeight() + "," + getBpp());
    }
  }
