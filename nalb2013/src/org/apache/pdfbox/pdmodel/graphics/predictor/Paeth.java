...
/**
 * From http://www.w3.org/TR/PNG-Filters.html: The Paeth filter computes a simple linear function of the three neighboring pixels (left, above, upper
 * left), then chooses as predictor the neighboring pixel closest to the computed value. This technique is due to Alan W. Paeth [PAETH].
 * 
...
public class Paeth extends PredictorAlgorithm {
   ...
   * 
   * This function is taken almost directly from the PNG definition on http://www.w3.org/TR/PNG-Filters.html
   * 
   * @param a left
   * @param b above
   * @param c upper left
  ...
  public void encodeLine(byte[] src, byte[] dest, int srcDy, int srcOffset, int destDy, int destOffset) {
    ...
    for (int x = 0; x < bpl; x++) {
      dest[x + destOffset] = (byte) (src[x + srcOffset] - paethPredictor(leftPixel(src, srcOffset, srcDy, x), abovePixel(src, srcOffset, srcDy, x),
          aboveLeftPixel(src, srcOffset, srcDy, x)));
    }
  }
  ...
  public void decodeLine(byte[] src, byte[] dest, int srcDy, int srcOffset, int destDy, int destOffset) {
    int bpl = getWidth() * getBpp();
    for (int x = 0; x < bpl; x++) {
      dest[x + destOffset] = (byte) (src[x + srcOffset] + paethPredictor(leftPixel(dest, destOffset, destDy, x),
          abovePixel(dest, destOffset, destDy, x), aboveLeftPixel(dest, destOffset, destDy, x)));
    }
  }
}
