...
public abstract class PredictorAlgorithm {
   ...
   * 
   * @param src raw image data
   * @param dest encoded data
   * @param srcDy byte offset between lines
   * @param srcOffset beginning of line data
   * @param destDy byte offset between lines
   * @param destOffset beginning of line data
   */
  ...
  public int aboveLeftPixel(byte[] buf, int offset, int dy, int x) {
    return (offset >= dy) && (x >= getBpp()) ? buf[(offset + x) - dy - getBpp()] : 0;
  }
