...
/**
 * This one took me a while, but i'm proud to say that it handles the appearance of a textbox. This allows you to apply a value to a field in the
 * document and handle the appearance so that the value is actually visible too. The problem was described by Ben Litchfield, the author of the
 * example: org.apache.pdfbox.examlpes.fdf.ImportFDF. So Ben, here is the solution.
 * 
 * @author sug
...
public class PDAppearance {
  ...
  /**
   * Returns the default apperance of a textbox. If the textbox does not have one, then it will be taken from the AcroForm.
   * 
  ...
  private void insertGeneratedAppearance(PDAnnotationWidget fieldWidget, OutputStream output, PDFont pdFont, List tokens,
  ...
  private PDFont getFontAndUpdateResources(List tokens, PDAppearanceStream appearanceStream) throws IOException {
    ...
    if (formResources != null) {
      ...
      if (da != null) {
        ...
        PDFStreamParser streamParser = new PDFStreamParser(new ByteArrayInputStream(data.getBytes("ISO-8859-1")), null);
        streamParser.parse();
  ...
  /**
   * Calculates where to start putting the text in the box. The positioning is not quite as accurate as when Acrobat places the elements, but it works
   * though.
   * 
