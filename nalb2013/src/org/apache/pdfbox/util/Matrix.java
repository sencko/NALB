...
public class Matrix implements Cloneable {
  static final float[] DEFAULT_SINGLE = { 1, 0, 0, 0, 1, 0, 0, 0, 1 };
  ...
  /**
   * This method multiplies this Matrix with the specified other Matrix, storing the product in the specified result Matrix. By reusing Matrix
   * instances like this, multiplication chains can be executed without having to create many temporary Matrix objects.
   * <p/>
   * It is allowed to have (other == this) or (result == this) or indeed (other == result) but if this is done, the backing float[] matrix values may
   * be copied in order to ensure a correct product.
   * 
   * @param other the second operand Matrix in the multiplication
   * @param result the Matrix instance into which the result should be stored. If result is null, a new Matrix instance is created.
  ...
  public float getXScale() {
     ...
     * 
     * The rotation matrix multiplied with the scaling matrix is: ( x 0 0) ( cos sin 0) ( x*cos x*sin 0) ( 0 y 0) * (-sin cos 0) = (-y*sin y*cos 0) (
     * 0 0 1) ( 0 0 1) ( 0 0 1)
     * 
     * So, if you want to deduce x from the matrix you take M(0,0) = x*cos and M(0,1) = x*sin and use the theorem of Pythagoras
     * 
     * sqrt(M(0,0)^2+M(0,1)^2) = sqrt(x2*cos2+x2*sin2) = sqrt(x2*(cos2+sin2)) = <- here is the trick cos2+sin2 is one sqrt(x2) = abs(x)
     */
