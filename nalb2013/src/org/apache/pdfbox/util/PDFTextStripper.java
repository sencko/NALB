...
/**
 * This class will take a pdf document and strip out all of the text and ignore the formatting and such. Please note; it is up to clients of this
 * class to verify that a specific user has the correct permissions to extract text from the PDF document.
 * 
 * The basic flow of this process is that we get a document and use a series of processXXX() functions that work on smaller and smaller chunks of the
 * page. Eventually, we fully process each page and then print it.
 * 
...
public class PDFTextStripper extends PDFStreamEngine {
  ...
  /**
   * The charactersByArticle is used to extract text by article divisions. For example a PDF that has two columns like a newspaper, we want to extract
   * the first column and then the second column. In this example the PDF would have 2 beads(or articles), one for each column. The size of the
   * charactersByArticle would be 5, because not all text on the screen will fall into one of the articles. The five divisions are shown below
   * 
  ...
  /**
   * Instantiate a new PDFTextStripper object. This object will load properties from PDFTextStripper.properties and will not do anything special to
  ...
  /**
   * Instantiate a new PDFTextStripper object. Loading all of the operator mappings from the properties object that is passed in. Does not convert the
   * text to more encoding-specific output.
   * 
  ...
  /**
   * Instantiate a new PDFTextStripper object. This object will load properties from PDFTextStripper.properties and will apply encoding-specific
  ...
  /**
   * Start a new article, which is typically defined as a column on a single page (also referred to as a bead). This assumes that the primary
   * direction of text is left to right. Default implementation is to do nothing. Subclasses may provide additional information.
   * 
  ...
  /**
   * Start a new article, which is typically defined as a column on a single page (also referred to as a bead). Default implementation is to do
  ...
  /**
   * This will print the text of the processed page to "output". It will estimate, based on the coordinates of the text, where newlines and word
   * spacings should be placed. The text will be sorted only if that feature was enabled.
   * 
  ...
  protected void writePage() throws IOException {
    ...
    for (int i = 0; i < charactersByArticle.size(); i++) {
      ...
      /*
       * Before we can display the text, we need to do some normalizing. Arabic and Hebrew text is right to left and is typically stored in its
       * logical format, which means that the rightmost character is stored first, followed by the second character from the right etc. However, PDF
       * stores the text in presentation form, which is left to right. We need to do some normalization to convert the PDF data to the proper logical
       * output format.
       * 
       * Note that if we did not sort the text, then the output of reversing the text is undefined and can sometimes produce worse output then not
       * trying to reverse the order. Sorting should be done for these languages.
       */
      ...
      while (textIter.hasNext()) {
        ...
        for (int a = 0; a < stringValue.length(); a++) {
          ...
          if ((dir == Character.DIRECTIONALITY_LEFT_TO_RIGHT) || (dir == Character.DIRECTIONALITY_LEFT_TO_RIGHT_EMBEDDING)
              || (dir == Character.DIRECTIONALITY_LEFT_TO_RIGHT_OVERRIDE)) {
            ltrCnt++;
          } else if ((dir == Character.DIRECTIONALITY_RIGHT_TO_LEFT) || (dir == Character.DIRECTIONALITY_RIGHT_TO_LEFT_ARABIC)
      ...
      /*
       * Now cycle through to print the text. We queue up a line at a time before we print so that we can convert the line from presentation form to
       * logical form (if needed).
       */
      List<TextPosition> line = new ArrayList<TextPosition>();

      textIter = textList.iterator(); // start from the beginning again
      /*
       * PDF files don't always store spaces. We will need to guess where we should add spaces based on the distances between TextPositions.
       * Historically, this was done based on the size of the space character provided by the font. In general, this worked but there were cases where
       * it did not work. Calculating the average character width and using that as a metric works better in some cases but fails in some cases where
       * the spacing worked. So we use both. NOTE: Adobe reader also fails on some of these examples.
       */
      ...
      while (textIter.hasNext()) {
        ...
        if ((lastPosition != null)
            && ((position.getFont() != lastPosition.getTextPosition().getFont()) || (position.getFontSize() != lastPosition.getTextPosition()
                .getFontSize()))) {
        ...
        /*
         * If we are sorting, then we need to use the text direction adjusted coordinates, because they were used in the sorting.
         */
        if (getSortByPosition()) {
        ...
        /*
         * Estimate the expected width of the space based on the average character width with some margin. This calculation does not make a true
         * average (average of averages) but we found that it gave the best results after numerous experiments. Based on experiments we also found
         * that .3 worked well.
         */
        ...
        if (lastPosition != null) {
          ...
          /*
           * XXX BC: In theory, this check should really check if the next char is in full range seen in this line. This is what I tried to do with
           * minYTopForLine, but this caused a lot of regression test failures. So, I'm leaving it be for now.
           */
          ...
          if ((expectedStartOfNextWordX != EXPECTEDSTARTOFNEXTWORDX_RESET_VALUE) && (expectedStartOfNextWordX < positionX) && (// only bother adding a
                                                                                                                               // space if the last
                                                                                                                               // character was not a
                                                                                                                               // space
              lastPosition.getTextPosition().getCharacter() != null) && !lastPosition.getTextPosition().getCharacter().endsWith(" ")) {
  ...
  protected void processTextPosition(TextPosition text) {
    ...
    if (showCharacter) {
      ...
      /*
       * In the wild, some PDF encoded documents put diacritics (accents on top of characters) into a separate Tj element. When displaying them
       * graphically, the two chunks get overlayed. With text output though, we need to do the overlay. This code recombines the diacritic with its
       * associated character if the two are consecutive.
       */
      if (textList.isEmpty()) {
        textList.add(text);
      } else {
        /*
         * test if we overlap the previous entry. Note that we are making an assumption that we need to only look back one TextPosition to find what
         * we are overlapping. This may not always be true.
         */
  ...
  /**
   * This is the page that the text extraction will start on. The pages start at page 1. For example in a 5 page PDF document, if the start page is 1
   * then all pages will be extracted. If the start page is 4 then pages 4 and 5 will be extracted. The default value is 1.
   * 
  ...
  /**
   * This will get the last page that will be extracted. This is inclusive, for example if a 5 page PDF an endPage value of 5 would extract the entire
   * document, an end page of 2 would extract pages 1 and 2. This defaults to Integer.MAX_VALUE such that all pages of the pdf will be extracted.
   * 
  ...
  /**
   * Set the desired line separator for output text. The line.separator system property is used if the line separator preference is not set explicitly
   * using this method.
   * 
  ...
  /**
   * Set the desired page separator for output text. The line.separator system property is used if the page separator preference is not set explicitly
   * using this method.
   * 
  ...
  /**
   * Set the desired word separator for output text. The PDFBox text extraction algorithm will output a space character if there is enough space
   * between two words. By default a space character is used. If you need and accurate count of characters that are found in a PDF document then you
   * might want to set the word separator to the empty string.
   * 
   * @param separator The desired page separator string.
   */
  ...
  /**
   * Character strings are grouped by articles. It is quite common that there will only be a single article. This returns a List that contains List
  ...
  /**
   * By default the text stripper will attempt to remove text that overlapps each other. Word paints the same character several times in order to make
   * it look bold. By setting this to false all text will be extracted, which means that certain sections will be duplicated, but better performance
   * will be noticed.
   * 
  ...
  /**
   * The order of the text tokens in a PDF file may not be in the same as they appear visually on the screen. For example, a PDF writer may write out
   * all text by font, so all bold or larger text, then make a second pass and write out the normal text.<br/>
   * The default is to <b>not</b> sort by position.<br/>
   * <br/>
   * A PDF writer could choose to write each character in a different order. By default PDFBox does <b>not</b> sort the text tokens before processing
  ...
  /**
   * Get the current space width-based tolerance value that is being used to estimate where spaces in text should be added. Note that the default
  ...
  /**
   * Set the space width-based tolerance value that is used to estimate where spaces in text should be added. Note that the default value for this has
   * been determined from trial and error. Setting this value larger will reduce the number of spaces added.
   * 
  ...
  /**
   * Get the current character width-based tolerance value that is being used to estimate where spaces in text should be added. Note that the default
   * value for this has been determined from trial and error.
   * 
   * @return The current tolerance / scaling factor
   */
  ...
  /**
   * Set the character width-based tolerance value that is used to estimate where spaces in text should be added. Note that the default value for this
   * has been determined from trial and error. Setting this value larger will reduce the number of spaces added.
   * 
  ...
  /**
   * returns the multiple of whitespace character widths for the current text which the current line start can be indented from the previous line
   * start beyond which the current line start is considered to be a paragraph start.
   * 
  ...
  /**
   * sets the multiple of whitespace character widths for the current text which the current line start can be indented from the previous line start
   * beyond which the current line start is considered to be a paragraph start. The default value is 2.0.
   * 
  ...
  /**
   * the minimum whitespace, as a multiple of the max height of the current characters beyond which the current line start is considered to be a
   * paragraph start.
   * 
  ...
  /**
   * sets the minimum whitespace, as a multiple of the max height of the current characters beyond which the current line start is considered to be a
  ...
  /**
   * Reverse characters of a compound Arabic glyph. When getSortByPosition() is true, inspect the sequence encoded by one glyph. If the glyph encodes
   * two or more Arabic characters, reverse these characters from a logical order to a visual order. This ensures that the bidirectional algorithm
   ...
   */
  protected PositionWrapper handleLineSeparation(PositionWrapper current, PositionWrapper lastPosition, PositionWrapper lastLineStartPosition,
  ...
  /**
   * tests the relationship between the last text position, the current text position and the last text position that followed a line separator to
   * decide if the gap represents a paragraph separation. This should <i>only</i> be called for consecutive text positions that first pass the line
   * separation test.
   * <p>
   * This base implementation tests to see if the lastLineStartPosition is null OR if the current vertical position has dropped below the last text
   * vertical position by at least 2.5 times the current text height OR if the current horizontal position is indented by at least 2 times the current
   ...
   * @param lastLineStartPosition the last text position that followed a line separator. May be null.
  ...
  protected void isParagraphSeparation(PositionWrapper position, PositionWrapper lastPosition, PositionWrapper lastLineStartPosition,
    ...
    if (lastLineStartPosition == null) {
      ...
      float xGap = (position.getTextPosition().getXDirAdj() - lastLineStartPosition.getTextPosition().getXDirAdj());// do we need to flip this for
                                                                                                                    // rtl?
   ...
   * The list of Patterns tested against is given by the {@link #getListItemPatterns()} method. To add to the list, simply override that method (if
   * sub-classing) or explicitly supply your own list using {@link #setListItemPatterns(List)}.
   * 
   * @param pw position
  ...
  /**
   * a list of regular expressions that match commonly used list item formats, i.e. bullets, numbers, letters, Roman numerals, etc. Not meant to be
   * comprehensive.
   */
  private static final String[] LIST_ITEM_EXPRESSIONS = { "\\.", "\\d+\\.", "\\[\\d+\\]", "\\d+\\)", "[A-Z]\\.", "[a-z]\\.", "[A-Z]\\)", "[a-z]\\)",
      "[IVXL]+\\.", "[ivxl]+\\.",
   ...
   * <p>
   * Order of the supplied list of patterns is important as most common patterns should come first. Patterns should be strict in general, and all will
   * be used with case sensitivity on.
   * </p>
   * 
   ...
   */
  private StringBuilder normalizeAdd(LinkedList<WordWithTextPositions> normalized, StringBuilder lineBuilder, List<TextPosition> wordPositions,
      TextPosition text) {
  ...
  /**
   * Internal class that maps strings to lists of {@link TextPosition} arrays. Note that the number of entries in that list may differ from the number
