...
public class TextNormalize {
  ...
  /*
   * Adds non-decomposing diacritics to the hash with their related combining character. These are values that the unicode spec claims are equivalent
   * but are not mapped in the form NFKC normalization method. Determined by going through the Combining Diacritical Marks section of the Unicode spec
  ...
  /**
   * Takes a line of text in presentation order and converts it to logical order. For most text other than Arabic and Hebrew, the presentation and
   * logical orders are the same. However, for Arabic and Hebrew, they are different and if the text involves both RTL and LTR text then the Unicode
   * BIDI algorithm must be used to determine how to map between them.
   * 
