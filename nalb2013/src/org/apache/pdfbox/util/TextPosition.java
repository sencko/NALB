...
public class TextPosition {
  /*
   * TextMatrix for the start of the text object. Coordinates are in display units and have not been adjusted.
   */
   ...
   */
  public TextPosition( PDPage page, Matrix textPositionSt, Matrix textPositionEnd, float maxFontH, float[] individualWidths, float spaceWidth,
      String string, PDFont currentFont, float fontSizeValue, int fontSizeInPt, float ws) {
   ...
   */
  @Deprecated
  public TextPosition( int pageRotation, float pageWidthValue, float pageHeightValue, Matrix textPositionSt, Matrix textPositionEnd, float maxFontH,
      float individualWidth, float spaceWidth, String string, PDFont currentFont, float fontSizeValue, int fontSizeInPt) {
    this(pageRotation, pageWidthValue, pageHeightValue, textPositionSt, textPositionEnd.getXPosition(), textPositionEnd.getYPosition(), maxFontH,
        individualWidth, spaceWidth, string, null, currentFont, fontSizeValue, fontSizeInPt);
  }
   ...
   * @param fontSizeInPt The font size in pt units.
   * 
   * @deprecated use {@link #TextPosition(int, float, float, Matrix, float, float, float, float, float, String, int[], PDFont, float, int)} insetad
   */
  @Deprecated
  public TextPosition( int pageRotation, float pageWidthValue, float pageHeightValue, Matrix textPositionSt, float endXValue, float endYValue,
      float maxFontH, float individualWidth, float spaceWidth, String string, PDFont currentFont, float fontSizeValue, int fontSizeInPt) {
    this(pageRotation, pageWidthValue, pageHeightValue, textPositionSt, endXValue, endYValue, maxFontH, individualWidth, spaceWidth, string, null,
        currentFont, fontSizeValue, fontSizeInPt);
  }
   ...
   */
  public TextPosition( int pageRotation, float pageWidthValue, float pageHeightValue, Matrix textPositionSt, float endXValue, float endYValue,
      float maxFontH, float individualWidth, float spaceWidth, String string, int[] codePoints, PDFont currentFont, float fontSizeValue,
      int fontSizeInPt) {
    this.textPos = textPositionSt;
  ...
  /**
   * Return the X starting coordinate of the text, adjusted by the given rotation amount. The rotation adjusts where the 0,0 location is relative to
   * the text.
   * 
  ...
  /**
   * This will get the text direction adjusted x position of the character. This is adjusted based on text direction so that the first character in
   * that direction is in the upper left at 0,0.
   * 
  ...
  /**
   * This will get the y position of the character with 0,0 in lower left. This will be adjusted by the given rotation.
   * 
  ...
  /**
   * This will get the width of a space character. This is useful for some algorithms such as the text stripper, that need to know the width of a
   * space character.
   * 
  ...
  public boolean contains(TextPosition tp2) {
    ...
    /*
     * No Y overlap at all so return as soon as possible. Note: 0.0 is in the upper left and y-coordinate is top of TextPosition
     */
    if (((tp2.getYDirAdj() + tp2.getHeightDir()) < getYDirAdj()) || (tp2.getYDirAdj() > (getYDirAdj() + getHeightDir()))) {
      return false;
    }
    /*
     * We're going to calculate the percentage of overlap. If its less than a 15% x-coordinate overlap then we'll return false because its negligible.
  ...
  /**
   * Merge a single character TextPosition into the current object. This is to be used only for cases where we have a diacritic that overlaps an
   * existing TextPosition. In a graphical display, we could overlay them, but for text extraction we need to merge them. Use the contains() method to
  ...
  public void mergeDiacritic(TextPosition diacritic, TextNormalize normalize) {
    ...
    for (int i = 0; (i < strLen) && !wasAdded; i++) {
      ...
      /*
       * This is the case where there is an overlap of the diacritic character with the current character and the previous character. If no previous
  ...
  private void insertDiacritic(int i, TextPosition diacritic, TextNormalize normalize) {
    /*
     * we add the diacritic to the right or left of the character depending on the direction of the character. Note that this is only required because
     * the text is currently stored in presentation order and not in logical order.
     */
  ...
  public boolean isDiacritic() {
    ...
    return ((type == Character.NON_SPACING_MARK) || (type == Character.MODIFIER_SYMBOL) || (type == Character.MODIFIER_LETTER));
  }
}